...
int time = 1500 ;     //tempo de barra pressionada deve ser >= a essa variável (em ms)
int countt=0;         // contador de trials corretos consecutivos 
int InfraRed= 7;      //variável de entrada do infravermelho
byte previous = LOW;  //status do nose poke no momento anterios 
byte debounceState = HIGH;  // status do nose poke, comparacao para aplicacao do debounce
...
unsigned long debounceDelay = 100;    //delay para realizar a comparação e verificar se o rato realmente saiu do nose poke // exclui alteraçoes que ocorrem abaixo de 50 ms
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
if (buttonpress != previous){             // compara o status atual com o anterios 
  if (flagdebounce==1){//verifica se o rato permaneceu dentro do nose poke
  	if (buttonpress == HIGH){ // se o sinal do IF for baixo (Rato cortou o feixe de IF) e antes era alto, entao guardar o tempo onde ocorreu
...
if (buttonpress == LOW){          // verifica se o rato saiu do nose poke sem debouncing
    digitalWrite(13,LOW);         // manda para a tdt o tempo real do offset sem debouncing
}
else{
  	lastDebounceTime = millis();  // salva o tempo em que foi detectado High Pela última vez
  	digitalWrite(13,HIGH);        // manda para a tdt o tempo real do offset 
}
...
if(millis() - lastDebounceTime > debounceDelay){// insere um atraso de 50 ms // para verificar se de fato o rato saiu do nose poke
    ...
    //debounceState = buttonpress;// atribui ao debounceState o valot atual do buttonpress 
  if(flagdebounce==0){
	  if (buttonpress == LOW){// se após 50ms ocorreu te fato a alteração para HIGH // o rato saiu do nose poke por um tempo superior a 50ms
